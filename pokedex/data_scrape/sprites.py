...
from pokedex.common.constants import POKEMONDB_URL, NATIONAL_DEX_URL, SPRITE_URL
from pokedex.common.constants import GAME_DICT_LIST, GEN_LIST

def scrape_names(url):
    """
    Takes 'https://pokemondb.net/pokedex/national' and returns
    a names list and a url list for each pokemon in the national 
    dex as it is used by pokemondb for navigation. 
    """
    soup = make_soup(url, 'html.parser')
    ...
    # return lists
    return pkmn_names, pkmn_page_url

def scrape_sprites(pkmn_names, display_output= False, record_time= True):
    """
    ...
    for pkmn in pkmn_names:
        ...
        # gives empty dict for each pokemon
        sprites_dict = GAME_DICT_LIST

        # Sprite page url, updated to new pokemon each loop
        PKMN_SPRITE_URL = f'{SPRITE_URL}/{pkmn}'
